...
1. **Generate the Xcode project**
   ...
   This reads `project.yml` and creates `CodeRelay.xcodeproj`.

2. **Build and run**

   Open `CodeRelay.xcodeproj` in Xcode, select the `CodeRelayApp` scheme,
   choose an iOS 17+ Simulator or device, and press Cmd+R.

After modifying `CodeRelayClient` or `CodeRelayKit` sources, rebuild the
...
```
CodeRelayApp/
  CodeRelayApp.swift              -- @main App entry point
  Models/
