...
- **CLI client-side** (`ConfigSetCommand`): rejects unknown keys, out-of-range ports, invalid log levels, negative `maxSessionsPerToken`, and unreadable `tlsCert`/`tlsKey` paths before shipping to the admin API. Note the switch matches on `(key, ConfigValue)` pairs, so a **non-numeric** port infers as `.string`, misses the `.int` case, and falls through to the server to reject — client-side type checking is a fast-path, not a guarantee. `claude-relay config validate` is a **narrower, server-dependent** check: it fetches `/config` over the admin API (so it needs the service running) and only verifies that a port which *does* parse as an integer falls in 1–65535, plus `wsPort != adminPort`. A non-numeric port is silently skipped, so a clean `validate` does not prove the config is loadable; its port bound is also looser than the server's own 1024–65535.
...
**Performance**: The foreground-process poll runs at 1 s for an attached session (`PTYSession.attachedPollCadence`) and 5 s once detached (`detachedPollCadence`). `SessionManager` switches cadence in exactly two places — `attachSession` and `detachSession`. **Known gap:** `resumeSession` reaches `.activeAttached` without restoring the fast cadence, so a session resumed after a detach keeps polling at 5 s until a real `attachSession`. Tab switches go through resume, so this is the common path, not the rare one. ANSI regex processing is skipped on the hot output path when no agent is running. `CodingAgent.processNames` is pre-lowercased at init so polling doesn't re-allocate strings on every tick.
