...
class PairingUrlTest {
    ...
    @Test fun `malformed percent-encoding with invalid hex returns null`() {
        assertNull(PairingURL.parse("coderelay://pair?host=%ZZ&port=9200&tls=0&code=K7QP2M4X"))
    }
    ...
    @Test fun `valid percent-encoded host parses successfully`() {
        val u = PairingURL.parse("coderelay://pair?host=a%2Eb&port=9200&tls=0&code=K7QP2M4X")!!
