...
Run every script **from this directory** (`CodeRelayAndroid/ml/`) so the
relative source-model paths resolve:

```bash
cd CodeRelayAndroid/ml
...
Relative to `CodeRelayAndroid/ml/`, under
`../../Sources/CodeRelaySpeech/Resources/`:
...
`../../Tests/CodeRelaySpeechTests/Fixtures`, with an optional `labels.json`
