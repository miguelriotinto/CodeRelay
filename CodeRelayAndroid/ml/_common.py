...
# Paths are relative to THIS file (CodeRelayAndroid/ml/), reaching back up to
# the SwiftPM target's Resources directory.
_HERE = os.path.dirname(os.path.abspath(__file__))
_RESOURCES = os.path.normpath(
    os.path.join(_HERE, "..", "..", "Sources", "CodeRelaySpeech", "Resources")
)
...
def require_source(path: str) -> str:
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(
            ...
            "Run these scripts from CodeRelayAndroid/ml/ on a Mac with the "
            "in-repo Sources/CodeRelaySpeech/Resources/ present."
        )
    return path
