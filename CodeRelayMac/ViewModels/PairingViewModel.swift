...
final class PairingViewModel: ObservableObject {
    ...
    func pair() async -> ConnectionConfig? {
        ...
        let controller = PairingController(
            store: CodeRelayMacApp.savedConnections,
    ...
    static func message(for error: PairingError, host: String, useTLS: Bool) -> String {
        ...
        case .unreachable:
            return "Could not reach \(host). Check the network and that the server is running."
        case .timedOut:
            return "The server did not respond. Check `claude-relay status`."
