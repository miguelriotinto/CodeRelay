...
*same source files*, from their existing location under `CodeRelayAndroid/`.
...
Nothing under `CodeRelayAndroid/` is modified by this build.
...
```
shared-protocol ─┐
shared-net       ├─ compiled from CodeRelayAndroid/, unmodified
shared-session   │
shared-terminal ─┘
...
live under `CodeRelayAndroid/` and are compiled here unchanged.
